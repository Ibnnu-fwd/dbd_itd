        ...
        <form action="{{ route('admin.ksh.detail.update', $data->id) }}" method="POST">
            ...
            <div class="sm:grid sm:grid-cols-2 sm:gap-4">
                <div>
                    ...
                    <x-input id="house_name" name="house_name" label="Nama Rumah" required class="max-w-xs" :value="$data->house_name" />
                    <x-input id="house_owner" name="house_owner" label="Nama Pemilik Rumah" required class="max-w-xs" :value="$data->house_owner" />
                    ...
                    <x-input id="tpa_description" name="tpa_description" label="Deskripsi Tpa" required class="max-w-xs" :value="$data->tpa_description" />
                    <div>
                        <p class="mb-2 text-xs 2xl:text-sm">Status Larva <span class="text-red-500">*</span></p>
                        <div class="flex items-center">
                            <div class="flex items-center mr-2">
                                <input id="positive-list-radio-license" type="radio" value="1" @if ($data->larva_status == 1) checked @endif name="larva_status"
                                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500" />
                                <label for="positive-list-radio-license" class="w-full py-3 ml-2 text-xs 2xl:text-sm font-medium text-gray-900 dark:text-gray-300">Ada</label>
                            </div>
                            <div class="flex items-center">
                                <input id="negative-list-radio-license" type="radio" value="0" @if ($data->larva_status == 0) checked @endif name="larva_status"
                                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500" />
                                <label for="negative-list-radio-license" class="w-full py-3 ml-2 text-xs 2xl:text-sm font-medium text-gray-900 dark:text-gray-300">
                                    Tidak Ada
                                </label>
                            </div>
                        </div>
                ...
                <div>
                    ...
                    <div class="sm:grid grid-cols-2 gap-x-4">
                        <x-input id="latitude" label="Latitude" name="latitude" type="text" required :value="$data->latitude" />
                        <x-input id="longitude" label="Longitude" name="longitude" type="text" required :value="$data->longitude" />
                    </div>
